...
function MessageItem({ message, index, messages, selectedConvo, lastMessageStatus }: MessageItemProps) {
    ...
    <div className={cn('flex gap-2 message-bounce mt-1', message.isOwn ? 'justify-end' : 'justify-start')}>
      ...
      {!message.isOwn && (
        <div className="w-8">
          {isGroupBreak && (
            <UserAvatar
              type="chat"
              name={participant?.displayName ?? 'Moji user'}
      ...
      <div className={cn('max-w-xs lg:max-w-md space-y-1 flex flex-col', message.isOwn ? 'items-end' : 'items-start')}>
        <Card className={cn('p-3', message.isOwn ? 'chat-bubble-sent border-0' : 'chat-bubble-received')}>
          <p className="text-sm leading-relaxed bread-words">{message.content}</p>
        </Card>
        {/* time */}
        {isGroupBreak && (
        ...
        {message.isOwn && message._id === selectedConvo.lastMessage?._id && (
          <Badge
            variant="outline"
            className={cn(
              'text-xs px-1 py-0.5 h-4 border-0',
